...
df_meta <- read.csv("C:/Users/Оля/Documents/BioStat_2023/namess.csv", sep = ",", header = TRUE, fileEncoding = "UTF-8")


df <- openxlsx2::read_xlsx("C:/Users/Оля/Documents/BioStat_2023/Donor Immune cells (1).xlsx", sheet = 1)  
...
##Подготовка отдельного датасета
...
diagnoz <- data.frame("Диагноз" = ifelse(main_df$d_z %in% c("ОЛЛ", "ОМЛ"), "ОЛ",
                                       ifelse(main_df$d_z %in% c("МДС", "ПМФ", "ХМЛ", "ЮММЛ"), "МПЗ", "ЛПЗ")))
...
null_model <- main_df %>%
  select(restadirovanie_na_moment_tkm_stadia_pred_tk_me, vozrast_donora, vozrast_polnyh_let_na_moment_tkm, rezim_kondicionirovania_pred_tk_me, sovmestimost_donora_recipienta_ptkm_pred_tk_me, vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm, cd34_kg_x10_ptkm_pdi)

#Объединяем в отдельный датасет
cox <-mutate(donor_cells_abs, outcomes, diagnoz, null_model)
...
#factor вместо character
cox <- cox %>%
  mutate_at(c('Диагноз', 'rezim_kondicionirovania_pred_tk_me', 'sovmestimost_donora_recipienta_ptkm_pred_tk_me', 'vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm'), as.factor)

str(cox)

describtion <- data.frame(names = names(cox)) %>%  
  left_join(., df_meta %>% select(cleaned_names, Describtion), 
            by = c("names" = "cleaned_names")) 

#Перевели 10^9 в 10^7
cox <- cox %>% 
  mutate(across(matches("cd"), ~ .x * 100L))
    
```

## OS Построение нулевой модели 

```{r}
library("survival")
library("survminer")

os.cox <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi, data =  cox)
summary(os.cox)

ggsurvplot(survfit(os.cox, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())

ggforest(os.cox, data = cox, cpositions = c(0.01, 0.22, 0.4))

```

## OS Добавление предикторов 

```{r}
#CD3
os.cox_cd3 <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd3_abs, data =  cox)
summary(os.cox_cd3)

ggsurvplot(survfit(os.cox_cd3, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(os.cox_cd3)
```
...
```{r}
#CD4
os.cox_cd4 <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd4_abs, data =  cox)
summary(os.cox_cd4)
...
```{r}
#CD8
os.cox_cd8 <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd8_abs, data =  cox)
summary(os.cox_cd8)

ggsurvplot(survfit(os.cox_cd8, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(os.cox_cd8)
```
...
```{r}
#CD19
os.cox_cd19 <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd19_abs, data =  cox)
summary(os.cox_cd19)

ggsurvplot(survfit(os.cox_cd19, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(os.cox_cd19)
```

```{r}
#CD56
os.cox_cd56 <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd56_abs, data =  cox)
summary(os.cox_cd56)

ggsurvplot(survfit(os.cox_cd19, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(os.cox_cd56)
```

```{r}
#CD16
os.cox_cd16 <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd16_abs, data =  cox)
summary(os.cox_cd16)

ggsurvplot(survfit(os.cox_cd16, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(os.cox_cd16)
```

```{r}
#CD56CD16
os.cox_cd56cd16 <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd56cd16_abs, data =  cox)
summary(os.cox_cd56cd16)

ggsurvplot(survfit(os.cox_cd56cd16, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(os.cox_cd56cd16)
```

```{r}
#CD4CD8
os.cox_cd4cd8 <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd4cd8_abs, data =  cox)
summary(os.cox_cd4cd8)

ggsurvplot(survfit(os.cox_cd4cd8, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(os.cox_cd4cd8)
```

```{r}
#CD3CD56
os.cox_cd3cd56 <- coxph(Surv(os2time, os2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd3cd56_abs, data =  cox)
summary(os.cox_cd3cd56)

ggsurvplot(survfit(os.cox_cd3cd56, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(os.cox_cd3cd56)
```
## EFS Построение нулевой модели

```{r}
efs.cox <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi, data =  cox)
summary(os.cox)

ggsurvplot(survfit(efs.cox, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())

ggforest(efs.cox)
```

## EFS Добавление предикторов

```{r}
#CD3
efs.cox_cd3 <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd3_abs, data =  cox)
summary(efs.cox_cd3)

ggsurvplot(survfit(efs.cox_cd3, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(efs.cox_cd3)
```

```{r}
#CD4
efs.cox_cd4 <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd4_abs, data =  cox)
summary(efs.cox_cd4)

ggsurvplot(survfit(efs.cox_cd3, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(efs.cox_cd4)
```

```{r}
#CD8
efs.cox_cd8 <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd8_abs, data =  cox)
summary(efs.cox_cd8)

ggsurvplot(survfit(efs.cox_cd8, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(efs.cox_cd8)
```

```{r}
#CD19
efs.cox_cd19 <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd19_abs, data =  cox)
summary(efs.cox_cd19)

ggsurvplot(survfit(efs.cox_cd19, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(efs.cox_cd19)
```

```{r}
#CD56
efs.cox_cd56 <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd56_abs, data =  cox)
summary(efs.cox_cd56)

ggsurvplot(survfit(efs.cox_cd19, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(efs.cox_cd56)
```

```{r}
#CD16
efs.cox_cd16 <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd16_abs, data =  cox)
summary(efs.cox_cd16)

ggsurvplot(survfit(efs.cox_cd16, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(efs.cox_cd16)
```

```{r}
#CD56CD16
efs.cox_cd56cd16 <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd56cd16_abs, data =  cox)
summary(efs.cox_cd56cd16)

ggsurvplot(survfit(efs.cox_cd56cd16, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(efs.cox_cd56cd16)
```

```{r}
#CD4CD8
efs.cox_cd4cd8 <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd4cd8_abs, data =  cox)
summary(efs.cox_cd4cd8)

ggsurvplot(survfit(efs.cox_cd4cd8, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(efs.cox_cd4cd8)
```

```{r}
#CD3CD56
efs.cox_cd3cd56 <- coxph(Surv(efs2time, efs2) ~ Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi + cd3cd56_abs, data =  cox)
summary(efs.cox_cd3cd56)

ggsurvplot(survfit(efs.cox_cd3cd56, data = cox), color = "#2E9FDF",
           ggtheme = theme_minimal())
ggforest(efs.cox_cd3cd56)
```


# Harrell's C-index OS
```{r}
library(survival)

summary(os.cox)$concordance[1]

Name <- c("Null-model", "+CD3", "+CD4", "+CD8", "+CD19", "+CD16", "+CD56", "+CD56CD16", "+CD4CD8", "+CD3CD56")
Cindex <- c(summary(os.cox)$concordance[1], summary(os.cox_cd3)$concordance[1], summary(os.cox_cd4)$concordance[1], summary(os.cox_cd8)$concordance[1], summary(os.cox_cd19)$concordance[1], summary(os.cox_cd56)$concordance[1], summary(os.cox_cd16)$concordance[1], summary(os.cox_cd56cd16)$concordance[1], summary(os.cox_cd4cd8)$concordance[1], summary(os.cox_cd3cd56)$concordance[1])

Cindex_os <- data.frame(Name, Cindex)
Cindex_os
```


# Harrell's C-index EFS
```{r}
library(survival)
#install.packages("pec")
library(pec)

Name <- c("Null-model EFS", "+CD3", "+CD4", "+CD8", "+CD19", "+CD16", "+CD56", "+CD56CD16", "+CD4CD8", "+CD3CD56")
Cindex <- c(summary(efs.cox)$concordance[1], summary(efs.cox_cd3)$concordance[1], summary(efs.cox_cd4)$concordance[1], summary(efs.cox_cd8)$concordance[1], summary(efs.cox_cd19)$concordance[1], summary(efs.cox_cd56)$concordance[1], summary(efs.cox_cd16)$concordance[1], summary(efs.cox_cd56cd16)$concordance[1], summary(efs.cox_cd4cd8)$concordance[1], summary(efs.cox_cd3cd56)$concordance[1])

Cindex_efs <- data.frame(Name, Cindex)
Cindex_efs
```

```{r}
Name <- c("Null-model OS", "+CD3", "+CD4", "+CD8", "+CD19", "+CD16", "+CD56", "+CD56CD16", "+CD4CD8", "+CD3CD56")
Cindex <- c(summary(os.cox)$concordance[1], summary(os.cox_cd3)$concordance[1], summary(os.cox_cd4)$concordance[1], summary(os.cox_cd8)$concordance[1], summary(os.cox_cd19)$concordance[1], summary(os.cox_cd56)$concordance[1], summary(os.cox_cd16)$concordance[1], summary(os.cox_cd56cd16)$concordance[1], summary(os.cox_cd4cd8)$concordance[1], summary(os.cox_cd3cd56)$concordance[1])
...
set.seed(3451)
train<- data[indices,]
res.cox<- coxph(Surv(os2time,os2)~Диагноз+restadirovanie_na_moment_tkm_stadia_pred_tk_me + vozrast_donora + vozrast_polnyh_let_na_moment_tkm + rezim_kondicionirovania_pred_tk_me + sovmestimost_donora_recipienta_ptkm_pred_tk_me +  vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm + cd34_kg_x10_ptkm_pdi, data=cox) # fitting cox model
train_predict <- predict(res.cox,newdata = cox,type = 'risk') #getting train prediction
cindex_train<- cIndex(cox$os2time, event = cox$os2, train_predict, weight = NULL) #getting concordance index
return(cindex_train[1])}

cox_train<- cox %>% 
  select(os2, os2time, Диагноз, restadirovanie_na_moment_tkm_stadia_pred_tk_me,  vozrast_donora, vozrast_polnyh_let_na_moment_tkm, rezim_kondicionirovania_pred_tk_me, sovmestimost_donora_recipienta_ptkm_pred_tk_me, vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm, cd34_kg_x10_ptkm_pdi) #train data frame with variables and endpoint
cox_test<- X_test # test data frame with variables and endpoint 
train_boots <- boot(data=cox_train, statistic=bootstrap_train, R=2000)
