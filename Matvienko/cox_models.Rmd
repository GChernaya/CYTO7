...
simplier$restadirovanie_na_moment_tkm_stadia_pred_tk_me[93] <- "ремиссия"
simplier$restadirovanie_na_moment_tkm_stadia_pred_tk_me[69] <- "ремиссия"
simplier$restadirovanie_na_moment_tkm_stadia_pred_tk_me[62] <- "рецидив"
simplier$restadirovanie_na_moment_tkm_stadia_pred_tk_me[59] <- "ремиссия"
simplier$restadirovanie_na_moment_tkm_stadia_pred_tk_me[15] <- "ремиссия"
simplier$restadirovanie_na_moment_tkm_stadia_pred_tk_me[61] <- "ремиссия"
simplier$restadirovanie_na_moment_tkm_stadia_pred_tk_me[60] <- "ремиссия"

patterns_to_remissiya <- c('хроническая фаза', 'хроническаяфаза', 'трансфрмация в ОМЛ, ремиссия','DIPSS 2-3, фиброзно-склеротическая стадия', 'РАИБ 2', 'DIPSSplus промежуточный-2 риск', 'РАИБ-2, очень высокий риск IPSS-R','РАИБ1, IPSS-R', 'стадия IIIB', 'стадияIIIB', 'стадия IIIB, ремиссия', 'стадияIIIB,ремиссия')
patterns_to_recidiv <- c('бластный криз', 'бластныйкриз', 'прогрессия', 'прогрессия в ОМЛ', 'рецидив, циторедукция')
...
## Корреляция/кластеризация

### Отдельные датасеты с субпопуляциями (абс. и %)
```{r echo=TRUE}
donor_cells_abs <- main_df %>%
  select(cd3_abs, cd4_abs, cd8_abs, cd19_abs, cd56_abs, cd16_abs,
         cd56cd16_abs, cd4cd8_abs, cd3cd56_abs)

donor_cells_perc <- main_df %>%
  select(cd3_perc, cd4_perc, cd8_perc, cd19_perc, cd56_perc, cd16_perc,
         cd56cd16_perc, cd4cd8_perc, cd3cd56_perc)
```
...
donor_cells_abs_corr <- cor(donor_cells_abs, method = "spearman", use = "pairwise.complete.obs")

corrplot(donor_cells_abs_corr, method = 'number')

donor_cells_perc_corr <- cor(donor_cells_perc, method = "spearman", use = "pairwise.complete.obs")

corrplot(donor_cells_perc_corr, method = 'number')
```
### Логарифмирование
```{r}
donor_cells_abs_ln <- log(donor_cells_abs[c(1:9)] + 1)
donor_cells_perc_ln <- log(donor_cells_perc[c(1:9)] + 1)
```


### Кластеризация (иерархическая), исходные данные
```{r echo=TRUE}
library(pheatmap)

donor_cells_abs_scaled <- scale(donor_cells_abs)
donor_cells_perc_scaled <- scale(donor_cells_perc) #Стандартизируем значения (переводим в z-шкалу)

#С эвклидовыми дистанциями
donor_cells_abs_dist <- dist(donor_cells_abs_scaled, 
                        method = "euclidean"
                        ) #Матрица дистанций

pheatmap(donor_cells_abs_scaled, 
         show_rownames = FALSE, 
         clustering_distance_rows = donor_cells_abs_dist,
         clustering_method = "ward.D2",
         cutree_cols = length(colnames(donor_cells_abs_scaled)),
         angle_col = 90, 
         main = "Dendrograms for clustering rows and columns with heatmap",
         border_color = NA)

donor_cells_perc_dist <- dist(donor_cells_perc_scaled, 
                        method = "euclidean"
                        )

pheatmap(donor_cells_perc_scaled, 
         show_rownames = FALSE, 
         clustering_distance_rows = donor_cells_perc_dist,
         clustering_method = "ward.D2",
         cutree_cols = length(colnames(donor_cells_perc_scaled)),
         angle_col = 90, 
         main = "Dendrograms for clustering rows and columns with heatmap",
         border_color = NA)
```
### Кластеризация (иерархическая), логарифмированные данные
```{r echo=TRUE}
library(pheatmap)

donor_cells_abs_ln_scaled <- scale(donor_cells_abs_ln)
donor_cells_perc_ln_scaled <- scale(donor_cells_perc_ln) #Стандартизируем значения (переводим в z-шкалу)

#С эвклидовыми дистанциями
donor_cells_abs_ln_dist <- dist(donor_cells_abs_ln_scaled, 
                        method = "euclidean"
                        ) #Матрица дистанций

pheatmap(donor_cells_abs_ln_scaled, 
         show_rownames = FALSE, 
         clustering_distance_rows = donor_cells_abs_ln_dist,
         clustering_method = "ward.D2",
         cutree_cols = length(colnames(donor_cells_abs_ln_scaled)),
         angle_col = 90, 
         main = "Dendrograms for clustering rows and columns with heatmap",
         border_color = NA)

donor_cells_perc_ln_dist <- dist(donor_cells_perc_ln_scaled, 
                        method = "euclidean"
                        )

pheatmap(donor_cells_perc_ln_scaled, 
         show_rownames = FALSE, 
         clustering_distance_rows = donor_cells_perc_ln_dist,
         clustering_method = "ward.D2",
         cutree_cols = length(colnames(donor_cells_perc_ln_scaled)),
         angle_col = 90, 
         main = "Dendrograms for clustering rows and columns with heatmap",
         border_color = NA)
```

# Подготовка к нулевым моделям
```{r}
main_df <- main_df %>%
  mutate("Диагноз" = case_when(
    str_detect(d_z, "ОЛЛ") ~ "ОЛ",
    str_detect(d_z, "ОМЛ") ~ "ОЛ",
    str_detect(d_z, "OMЛ") ~ "ОЛ",
    str_detect(d_z, "ЛХ") ~ "ЛПЗ",
    str_detect(d_z, "ХЛЛ") ~ "ЛПЗ",
    str_detect(d_z, "НХЛ") ~ "ЛПЗ",
    str_detect(d_z, "ПМФ") ~ "МПЗ",
    str_detect(d_z, "ХМЛ") ~ "МПЗ",
    str_detect(d_z, "МДС") ~ "МПЗ",
    str_detect(d_z, "ЮММЛ") ~ "МПЗ"))
...
main_df %>%
  select(`Диагноз`,restadirovanie_na_moment_tkm_stadia_pred_tk_me,  vozrast_donora, vozrast_polnyh_let_na_moment_tkm, vozrastnaa_gruppa, sovmestimost_donora_recipienta_ptkm_pred_tk_me, rezim_kondicionirovania_pred_tk_me, vid_tkm_nastoasej_ptkm_pred_tk_me_pditkm, cd34_kg_x10_ptkm_pdi) %>%
  tbl_summary()
```

# Нулевая модель оРТПХ
```{r}
coxph(
  Surv(a_gvhd24time, ifelse(a_gvhd24 == 1, 1, 0)) ~ vozrast_polnyh_let_na_moment_tkm +
    `Диагноз` +
  ...
  summary()
```
# Добавляем предикторы по одному
```{r}
coxph(
  Surv(a_gvhd24time, ifelse(a_gvhd24 == 1, 1, 0)) ~ vozrast_polnyh_let_na_moment_tkm +
    `Диагноз` +
...
# Нулевая модель хрРТПХ
```{r}
coxph(
  Surv(c_gvh_dtime, ifelse(c_gvhd == 1, 1, 0)) ~ vozrast_polnyh_let_na_moment_tkm +
    `Диагноз` +
    restadirovanie_na_moment_tkm_stadia_pred_tk_me +
    sovmestimost_donora_recipienta_ptkm_pred_tk_me +
    rezim_kondicionirovania_pred_tk_me +
